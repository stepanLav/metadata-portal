...
pub(crate) async fn export_specs_async(config: &AppConfig) -> Result<ExportData> {
    ...
    info!(
        "📊 Collecting data for {} chains (max 10 concurrent)",
        config.chains.len()
    );
    ...
    let results: Vec<_> = stream::iter(config.chains.iter())
        .map(|chain| {
            ...
            async move {
                ...
                let metadata_qrs =
                    match collect_metadata_qrs(&all_metadata, &chain.name, &live_meta_version) {
                        Ok(qrs) => qrs,
                        Err(e) => {
    ...
    info!(
        "📈 Summary: {}/{} chains collected successfully",
        export_specs.len(),
        config.chains.len()
    );
