...
mod tests {
    use super::*;
    use crate::lib::path::ContentType;
    use std::path::Path;
    ...
    fn return_latest_metadata() {
        ...
        let (result, _) = files.get("kusama").unwrap();
        assert_eq!(result.file_name.content_type, ContentType::Metadata(10));
    }
    ...
    fn return_specs() {
        ...
        let result = files.get("kusama").unwrap();
    ...
    fn prefer_signed_specs() {
        let path = Path::new("./for_tests/signed_specs");
